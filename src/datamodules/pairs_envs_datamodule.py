...
from src.datamodules.components.multi_envs import MultiEnv,MultiEnvCV
from src.datamodules.components.single_envs import SingleEnv,SingleEnvCV
import albumentations as A
from albumentations.pytorch import ToTensorV2

class PL_PairEnvs(LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if stage == "fit" or stage is None:
            train_ds = MultiEnv(root_dir=self.data_dir,
                                env1=self.train_env_1,
                                env2=self.train_env_2,
                                resize=self.resize,
                                extract_roi=self.extract_roi,
                                 masked=self.masked)

            n_val = int(len(train_ds)*self.val_split)
            n_test = int(len(train_ds)*self.test_split)
            idx_train = list(range(len(train_ds)))
            idx_val = list(range(len(train_ds)))
            

            train_ds = Subset(train_ds, idx_train[n_val:-n_test])
            val_ds = Subset(train_ds, idx_val[:n_val])
...
class PL_JointMultipleEnvsCV(LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        # called on every GPU
        if stage == "fit" or stage is None:
            self.train_ds= MultiEnvCV(root_dir=self.data_dir,
                                      env1=self.train_env_1,
                                      env2=self.train_env_2,
                                      resize=self.resize,
                                      extract_roi=self.extract_roi,
            ...
            self.val_ds = MultiEnvCV(root_dir=self.data_dir,
                                     env1=self.train_env_1,
                                     env2=self.train_env_2,
                                     resize=self.resize,
                                     extract_roi=self.extract_roi,
