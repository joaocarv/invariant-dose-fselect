...
def train_classifier(config: DictConfig) -> Optional[float]:
    ...
    # Correct data_dir in condig
    # config.data_dir = config.data_dir + '/' + config.name
    # config.data_dir =
    ...
    # path_selected_features = '/cluster/scratch/jcarvalho/multi-dose-fselect/logs/features/'+experiment_name
    print('config data dir', config.data_dir)
    path_selected_features = config.data_dir +'/selected_features.npy'
    model._set_classifier(z_size=len(np.load(path_selected_features)))

    config.name = config.name + '_classifier'
